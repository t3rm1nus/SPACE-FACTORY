...
| 38 | **El planner sobrescribía `books.title` con la idea/descripción cruda del usuario cuando caía a fallback determinista** (`_fallback_plan` devuelve `title==description==idea` por construcción) — el título real escrito en el formulario se perdía tras la fase planner. | Pérdida silenciosa del título del usuario en todo libro cuyo planner caiga a fallback (evidencia real: book_71, task 1893). | FIXED + VALIDATED (2026-08-28) | Media | Causa raíz: modules/book_planner/main.py:539/541 (_fallback_plan) + core/autopilot.py:813-817 (update_book_title incondicional tras la fase planner, presente desde el commit inicial fad41a0 sin justificación documentada del caso LLM-éxito). Fix: guard en core/autopilot.py — solo persiste el título del planner si es distinto de la idea/description del fallback. Archivos: core/autopilot.py, tests/test_editorial_metadata.py, tests/test_autopilot_persist_chapters.py. Tests: 18/18 PASS. Commit: a687c5a. **Limitación conocida:** el guard cubre el caso fallback (reportado); el caso planner LLM exitoso con título distinto de la idea SIGUE sobrescribiendo el título del usuario — pendiente de decisión del arquitecto (hallazgo punto 1, sesión 2026-08-28: no hay razón documentada para esa sobrescritura). |
