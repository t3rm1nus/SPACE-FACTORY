...
```text
809 passed, 0 failed, 0 errors, 1 skipped   (tests/ + modules/, incl. E2E runner) — 2026-08-29 (226.01s)
checkpoint post-commit 580e67c (§17 #38 título planner, §17 #30 cadena completa image_search/reset/overwrite, §17 #39 imágenes duplicadas + fuga meta-texto), sin regresión
```
...
| 30 | **Con image_search_ratio=1.0 (100% web), la cuota de compensación IA es 0 por diseño (fix 2026-08-25) — si algún slot de descarga web falla (403 Forbidden/timeout), el capítulo queda estructuralmente por debajo de image_count sin ningún colchón, y quality_gate (exige número EXACTO de imágenes por capítulo) falla.** Trade-off de diseño conocido, no bug del pipeline de imágenes. Evidencia real book_68 (retry 2026-08-26 21:11): 8/24 capítulos con 4/5 imágenes por fallos de descarga puntuales (403), overall_status=FAIL pese a que TODO lo demás del gate es PASS (imágenes existen, legibles, metadata presente; capítulos/fuentes/metadatos PASS). Tasks final_quality_control 1360/1361 done, job FAILED quality_gate attempts=2. | Libros con ratio=1.0 y descargas web imperfectas quedan bloqueados en quality_gate aunque el resultado sea funcionalmente válido; requiere reset manual de fases para reintentar (sin garantizar que los slots vuelvan a fallar igual). | FIXED + VALIDATED (2026-08-29) — E2E CONFIRMADO (book_72). Fix (cadena completa, en orden de implementación): 1. modules/image_search/main.py: paginación real vía SearXNG pageno, presupuesto IMAGE_SEARCH_TOTAL_TIME_BUDGET=90s + IMAGE_SEARCH_MAX_PAGES=6, dedupe entre páginas. 2. Guard de presupuesto también DENTRO del loop de candidatos de cada página (no solo entre páginas) — evita timeout de 170s a mitad de una página con muchos candidatos lentos/inválidos. 3. Guard de CUPO dentro del mismo loop (if slot >= requested: break) — evita procesar páginas enteras de miles de candidatos cuando el cupo ya se completó (bug que causó 109-140 imágenes/capítulo en pruebas intermedias). 4. _NON_RASTER_EXTENSIONS={".svg"}: descarte por extensión antes de descargar, sin gastar DOWNLOAD_TIMEOUT. 5. _ICON_LIBRARY_DENYLIST={"cdn.jsdelivr.net"}: descarta librerías de iconos (devicons, lucide-static) por denylist antes de evaluar, separado del denylist de marca/editoriales existente (§17 #5). 6. core/autopilot.py (_run_image_gen_split): ELIMINADA compensación con generate_chapter_images (IA) para shortfall — por decisión explícita del arquitecto, ninguna cuota de IA de respaldo; solo WARNING de diagnóstico. 7. modules/quality_control/main.py: tolerancia de déficit <=1 SOLO con image_search_ratio==1.0 (ya existente de una sesión previa, commit e354df1, validada aquí por primera vez end-to-end). 8. frontend/editorial.py (_build_book_dict): propaga image_search_ratio real al dict de quality_control (antes defaulteaba a 0.0, la tolerancia nunca se activaba). 9. core/autopilot.py (reset_from_phase): cuando from_phase es "image_gen" y se resetea el libro completo, fuerza subs_all=True (regenera TODOS los subs, incluso PASS-con-déficit) — a diferencia de otras fases donde PASS siempre es terminal y correcto. 10. core/autopilot.py (_persist_chapter, rama image_gen): persist_chapter_images ahora con overwrite=True (hay una única llamada por capítulo por ejecución tras eliminar la compensación IA del punto 6; sin esto, cada reset ACUMULABA imágenes de ejecuciones previas — llegó a 342 archivos/capítulo en pruebas). Archivos: modules/image_search/main.py, modules/image_search/module.json (timeout_seconds 120→170), core/autopilot.py, modules/quality_control/main.py, frontend/editorial.py. Tests: 25/25 PASS (image_search) + todos los focalizados de la sesión PASS (persist_chapters, editorial_panel, reset_from_phase); suite completa pendiente de checkpoint. Evidencia E2E: book_72 (bilingüe es/en, image_search_ratio=1.0, 10 capítulos) — job COMPLETED, quality_gate PASS (WARNING trazable «Imágenes por capítulo: 5 (tolerado, ratio=1.0 100% web)», 4/5 en los 10 capítulos), output/docx/book_72_es.docx (5.698.206 bytes) + output/docx/book_72_en.docx (5.700.081 bytes) generados 2026-08-29. | Baja | Próximo paso SIN autorizar aún — decidir entre: (a) relajar el gate a tolerancia N-1 (o umbral %) cuando image_search_ratio=1.0; (b) reservar una cuota IA mínima incluso con ratio=1.0 como red de seguridad (p.ej. round(num_images*(1-ratio)) >= 1); o (c) dejarlo como comportamiento esperado y documentar que el usuario ajuste el ratio manualmente (mitigación aplicada a book_67: UPDATE books SET image_search_ratio=0.8 → 1 imagen IA de margen/capítulo). Ninguna de las 3 toca código hasta decisión explícita del usuario. **Nueva evidencia (2026-08-28, book_71):** tras desbloquear research (§17 #37), el re-run E2E llegó a quality_gate y falló por déficit de imágenes (check FAIL con origin_phase=image_gen) — mismo patrón de este ítem, sin fix nuevo requerido en esta ronda. | | Ninguno — cierre completo de la cadena §17 #30. La decisión pendiente de 2026-08-28 queda resuelta: comportamiento esperado documentado (antigua opción c) + cadena de fixes anterior; en producción con ratio=1.0 la web aporta típicamente image_count-1 y el gate lo tolera como WARNING trazable. Checkpoint de suite completa pendiente para §14/§23. |
...
| 38 | **El planner sobrescribía `books.title` con el título generado por el LLM (o con la idea cruda en fallback), pisando el título que el usuario ya escribió en el formulario — obligatorio, §13.** | Pérdida silenciosa del título del usuario en todo libro (evidencia real: book_71, task 1893 — título perdido antes del fix; dato histórico NO corregido en BD, mismo criterio que book_37/39/57/67/68/69: se conserva intacto como evidencia, no se regenera). | **FIXED + VALIDATED (2026-08-28) — CERRADO POR COMPLETO** | Media | Causa raíz: llamada incondicional a `update_book_title()` en core/autopilot.py tras la fase planner, presente desde el commit inicial (fad41a0), sin justificación documentada. Fix: **eliminada por completo la llamada a `update_book_title()` tras la fase planner** (ambos casos: fallback y LLM-éxito); `books.title` queda intocable post-creación en todos los casos. `update_book_title_en` (columna distinta, title_en) NO se tocó — sigue funcionando igual. Archivos: core/autopilot.py, tests/test_editorial_metadata.py, tests/test_autopilot_persist_chapters.py. Tests: 18/18 PASS (incluye test renombrado test_planner_generated_title_does_not_overwrite_user_title). Commits: a687c5a (fix parcial fallback) + 5fe18c0 (fix completo, elimina la llamada por completo). Sin limitación conocida pendiente. |
| 39 | **Imágenes repetidas entre capítulos del mismo libro (misma foto física insertada como Figura N en varios capítulos distintos) y fuga ocasional de meta-texto del LLM ("Entendido. No reproduciré...") en la prosa final.** | Revisión del entregable (DOCX) muestra la misma imagen física en capítulos distintos (Figuras clónicas) y párrafos de acuse del LLM filtrados a prosa; degrada seriamente la calidad editorial percibida del libro final. | FIXED + VALIDATED (2026-08-29) — E2E CONFIRMADO (book_75). Evidencia causa: book_72.docx (revisión manual del entregable, 2026-08-29) — 51 archivos físicos en disco con solo 12 contenidos MD5 únicos; una imagen repetida 11 veces entre capítulos distintos. Causa: los 10 capítulos de book_72 compartían título genérico (planner en fallback, deuda ya documentada) → queries de búsqueda casi idénticas → mismos resultados top de SearXNG → misma imagen descargada en cada capítulo. Fix (2 partes independientes + detector): 1. Query diferenciada por capítulo: nuevo campo chapter_search_topic en ImageSearchPayload (core/schemas.py); core/autopilot.py (_run_image_gen_split) lo resuelve desde el primer heading usable del outline del capítulo (o objective como fallback), con fail-safe a la query histórica si no hay nada usable; modules/image_search/main.py (_search_query/_search_query_en) lo prioriza sobre chapter_title genérico. 2. Dedupe cross-chapter por hash SHA-1 (red de seguridad): registro persistente por libro en <IMAGE_STORAGE_ROOT>/books/<id>/chapters/_content_hashes.json; una imagen con el mismo hash ya usada en OTRO capítulo del mismo libro se descarta sin consumir slot de error (sigue intentando con el siguiente candidato); mismo hash en el propio capítulo se permite (re-ejecución intencional). 3. REFUSAL_PATTERNS ampliado (modules/chapter_writer/main.py y modules/editor/main.py, mismo patrón regex en ambos): detecta acuses de meta-instrucción tipo "Entendido. No reproduciré/generaré/proporcionaré..." SOLO a inicio de párrafo con negación en la misma línea (~240 chars) — evita falsos positivos con prosa legítima que use "entendido" en otro contexto. Archivos: core/schemas.py, core/autopilot.py, modules/image_search/main.py, modules/chapter_writer/main.py, modules/editor/main.py. Tests: 137/137 PASS (image_search + persist_chapters + chapter_writer_placeholder + editor + reset_from_phase). Evidencia E2E: book_75 ("Los inicios de la aviación comercial", 3 capítulos, ratio=1.0) — COMPLETED, 9/9 imágenes con hash MD5 único (antes book_72: 12 únicos de 51), queries confirmadas usando heading real del outline por capítulo (no título genérico), 0 fugas de meta-texto en el texto completo del DOCX. | Media | book_72.docx (12 MD5 únicos / 51 archivos; 1 imagen x11); book_75.docx (9/9 únicos, 9 media = 9 embeds) | Ninguno — cierre completo de la ronda. Nota de limitación conocida: el dedupe (parte 2) es red de seguridad; la causa semántica la ataca la parte 1, que requiere outline con headings reales (si el planner cae a fallback con outline genérico, la parte 2 evita repetición física pero las imágenes pueden seguir siendo temáticamente cercanas). |
| 40 | **Backstop determinista solapa secciones con el texto ya escrito por el LLM cuando hay continuaciones rechazadas por duplicado, causando párrafos casi-idénticos intra-capítulo** — el duplicate-continuation guard (§17 #8) rechaza correctamente las continuaciones repetidas del LLM, pero el relleno del backstop trabaja desde el outline genérico y puede regenerar contenido que solapa la prosa inicial del LLM (p.ej. la misma sección descrita dos veces con texto casi idéntico). El detector de secciones duplicadas del writer (duplicate_sections) no evalúa esa coherencia LLM↔backstop. | Párrafos/secciones casi-idénticos dentro del mismo capítulo en el DOCX final; no bloquea el job (quality_gate PASS). | OPEN (2026-08-29) — diagnosticado, SIN FIX: toca modules/chapter_writer/main.py (PROTECTED) y requiere diseño más cuidadoso (dedupe LLM↔backstop por similitud de secciones antes del relleno). | Media-alta | book_72 cap.3: sección "La era del realidad virtual (2016)" aparece dos veces con texto casi idéntico; metadata.json de chapter_3 (data/artifacts/72): duplicate_detected=true, continuation_rejected_as_duplicate=true, total_continuation_attempts=2, successful_continuations=0, initial_word_count=769 → final 1792 vía backstop (deterministic_used=true). | Diseño pendiente de autorización: (a) dedupe de secciones LLM↔backstop por similitud antes del relleno; (b) hacer que el backstop continúe desde el último heading NO escrito en vez de recorrer el outline completo; (c) pasar el texto LLM existente como contexto de exclusión al generador del backstop. |
...
TESTS:                   Checkpoint suite completa 2026-08-29 (post-sesión §17 #30/#38/#39): 809 passed, 0 failed, 0 errors, 1 skipped (226.01s, tests/ + modules/). Sin regresión tras la cadena completa de fixes de image_search (paginación, presupuesto, cupo, denylist iconos, dedupe cross-chapter), reset_from_phase forzado en image_gen, overwrite en persist_chapter_images, query por outline, y detector de fuga de meta-texto LLM.
...
FASE sesión 2026-08-29 completa: cierre cadena §17 #30 (E2E book_72) + ronda P1a/P1b/P2 (E2E book_75) / 2026-08-29
- cadena §17 #30 (COMMIT 718ba66): paginación SearXNG con presupuesto y guards intra-página de presupuesto/cupo; _NON_RASTER_EXTENSIONS (.svg); _ICON_LIBRARY_DENYLIST (cdn.jsdelivr.net); compensación IA eliminada (decisión del arquitecto); reset_from_phase fuerza subs_all con from_phase=image_gen; persist_chapter_images overwrite=True en rama image_gen; tolerancia ratio=1.0 (e354df1) validada end-to-end → book_72 COMPLETED, QC PASS con WARNING trazable, docx es+en ~5.7 MB.
- ronda P1a/P1b/P2: query de image_search diferenciada por capítulo (chapter_search_topic desde el heading del outline, core/schemas.py + core/autopilot.py + modules/image_search/main.py); dedupe cross-chapter por SHA-1 con registro persistente por libro (_content_hashes.json); REFUSAL_PATTERNS ampliado en chapter_writer y editor (acuse "Entendido. No reproduciré..." solo a inicio de párrafo con negación en línea).
- validación: 137/137 PASS focalizados; E2E book_75 COMPLETED con 9/9 imágenes MD5 únicas (book_72: 12 únicos de 51), queries por outline confirmadas, 0 fugas de meta-texto.
- abierto: §17 #40 (backstop solapa secciones con texto LLM; PROTECTED, sin fix).
- estado: CERRADO - FIXED + VALIDATED (E2E CONFIRMADO book_72 y book_75). Ver §17 #30, #39, #40 y §16.

FASE fix título-planner (§17 #38) / 2026-08-28
- qué se hizo: guard en core/autopilot.py — el título del planner solo se persiste si difiere de la idea/description del fallback determinista (title==description==idea); el título real del usuario ya no se pierde en libros con planner en fallback.
- validación: 18/18 PASS (test_editorial_metadata + test_autopilot_persist_chapters, incluye test nuevo del pisado post-planner). Evidencia real: book_71 (task 1893). Commit a687c5a. Limitación conocida: caso planner LLM exitoso sigue sobrescribiendo el título — pendiente de decisión del arquitecto.
- estado: CERRADO - VALIDADO (caso fallback); caso LLM-éxito abierto a decisión.
# 24. CHANGELOG RESUMIDO (hitos)

FASE cierre completo §17 #30 (cadena image_search + reset + overwrite) E2E VALIDADO con book_72 / 2026-08-29
- qué se hizo: paginación SearXNG con presupuesto (90s/6 páginas) y guards intra-página de presupuesto y cupo; _NON_RASTER_EXTENSIONS (.svg); _ICON_LIBRARY_DENYLIST (cdn.jsdelivr.net); eliminada compensación IA para shortfall (decisión del arquitecto); reset_from_phase fuerza subs_all con from_phase=image_gen; persist_chapter_images con overwrite=True en rama image_gen de _persist_chapter. La tolerancia ratio=1.0 de quality_control (commit e354df1, 2026-08-28) se valida aquí end-to-end por primera vez.
- validación: 25/25 PASS image_search + focalizados PASS (persist_chapters, editorial_panel, reset_from_phase); E2E real book_72 COMPLETED, quality_gate PASS (WARNING trazable), docx es+en ~5.7 MB. Resto de la cadena (sin commit previo) entra en el commit de este cierre; denylist jsdelivr con test nuevo incluido.
- estado: CERRADO - FIXED + VALIDATED (E2E CONFIRMADO). Ver §17 #30 y §16.

FASE cierre completo §17 #38 (title intocable post-creación) / 2026-08-28
- qué se hizo: eliminada por completo la llamada a update_book_title() tras la fase planner (ambos casos: fallback y LLM-éxito) — books.title queda intocable post-creación (título obligatorio §13); update_book_title_en intacta.
- validación: 18/18 PASS (tests focalizados editorial_metadata + autopilot_persist_chapters, test renombrado test_planner_generated_title_does_not_overwrite_user_title).
- estado: CERRADO POR COMPLETO - VALIDADO. Commits: a687c5a (parcial) + 5fe18c0 (completo).

FASE fix tolerancia quality_gate ratio=1.0 (§17 #30) / 2026-08-28
- qué se hizo: _check_images (modules/quality_control/main.py) tolera déficit de 1 imagen/capítulo SOLO con image_search_ratio==1.0 (100% web): el check pasa como WARNING trazable en vez de FAIL; déficit ≥2 o ratio≠1.0 mantienen la comparación exacta histórica.
- validación: 13/13 PASS en modules/quality_control/tests/test_quality_control.py (3 tests nuevos: tolera déficit 1, sigue fallando con déficit 2, ratio=0.5 sin tolerancia). Commit e354df1. Servidor activo verificado con el fix cargado (inicio posterior al commit).
- estado: CERRADO - VALIDADO. Evidencia previa: book_68, book_71.
