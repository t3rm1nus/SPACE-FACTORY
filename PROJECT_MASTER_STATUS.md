...
```text
764 passed, 0 failed, 0 errors, 1 skipped   (tests/ + modules/, incl. E2E runner) — 2026-08-27 (276.62s)
checkpoint post-commits sesión §17 #22/#23/#27/#28/#32/#33 + #35(F1-F3), sin regresión
```
...
| 36 | **Retry tras FAIL de quality_gate no vuelve a la fase de origen real** — retry_job es un reset plano: solo resetea fases con status no-PASS, nunca retrocede sobre fases ya PASS. Si quality_gate falla por un déficit generado en una fase anterior ya PASS (ej. image_gen con imágenes insuficientes, §17 #30), el retry solo re-ejecuta quality_gate, que vuelve a fallar por lo mismo — bucle estéril. quality_control tampoco expone hoy ningún campo estructurado de "fase responsable", solo mensajes de texto libre. | Bucle de retry sin progreso en libros con FAIL de quality_gate causado por fases anteriores (evidencia: book_67/68, §17 #30). Requiere resetear manualmente fases anteriores vía cirugía directa en BD (no soportado por API). | PARTIALLY ADDRESSED — diseño en 3 fases decidido 2026-08-28 (ver §24): (1) origin_phase estructurado en QC [EN CURSO], (2) resolver §17 #31 como precondición de seguridad antes de permitir reset de image_gen, (3) retry_job(from_phase=...) con cascade a fases dependientes + wiring API/front. | Media | Diagnóstico de sesión 2026-08-28 (solo lectura): retry_job (core/autopilot.py L.306-354) nunca retrocede sobre fases PASS; QualityControlItem (core/schemas.py L.475-479) solo tiene status+message, sin campo de origen. | Fase 1: añadir origin_phase a QualityControlItem y anotarlo en los ~15 puntos FAIL de quality_control (mapeo directo por check: images→image_gen, sources→research, chapters-text→writer/editor, documents→docx). |
...
| **P3** | `persist_chapter_images` (frontend/editorial.py) sobrescribe `chapters.images` en vez de fusionar; pierde imágenes previas válidas si un capítulo ya persistido se re-ejecuta. No bloqueante (el retry automático respeta subs PASS). Ref. §17 #31. | Deuda de diseño frágil (no-idempotente ante re-ejecución de capítulo persistido); no hay evidencia de producción real porque el motor nunca re-ejecuta capítulos completados. Sin fix por decisión explícita (ver §17 #31). |
...
TESTS:                   Checkpoint suite completa 2026-08-27 (post-commits §17 #22/#23/#27/#28/#32/#33 + #35 F1-F3): 764 passed, 0 failed, 0 errors, 1 skipped (276.62s, tests/ + modules/). Histórico: checkpoint 2026-08-26 (736 passed).
...
NEXT RECOMMENDED ACTION:          §17 #35 CERRADO COMPLETAMENTE (F1 error_type + F2 gate diferenciado + F3 marcador en DOCX, todas VALIDADAS 2026-08-27). book_69 abandonado (evidencia histórica, §17 #33). book_70 cap.538 (Claude/GTA, accuracy parcial) → ahora PASS_WITH_WARNING + marcador en DOCX en vez de tumbar el job. book_59 sigue bloqueado dura. Próximo paso: validación end-to-end del front (book_70 retry con cap.538 accuracy parcial → job COMPLETED con capítulo marcado).
